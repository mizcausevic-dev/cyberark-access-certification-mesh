// Structure + data-integrity tests for CyberArk Access Certification Mesh
...
test("HTML5 doctype + lang + title", () => {
  ...
  assert.match(html, /<title>CyberArk\s+Access\s+Certification\s+Mesh/);
});
...
test("4 stat tiles match synthetic data", () => {
  ...
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">30<\/span><span class="stat-label">Total\ safes/);
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">9<\/span><span class="stat-label">Overdue\ certs/);
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">11<\/span><span class="stat-label">In\ review/);
  assert.match(html, /<span class="stat-num(?:\s+(?:ok|warn|crit))?">783<\/span><span class="stat-label">Member\ entitlements/);
});
...
test("First anomaly card content present (data-integrity proxy)", () => {
  assert.ok(html.includes("Owner is also a privileged member"), "First anomaly substring missing from rendered HTML");
});
...
test("First audit event kind present (data-integrity proxy)", () => {
  assert.ok(html.includes("cert.cyberark.cert-started"), "First audit event kind missing from rendered HTML");
});
...
test("CNAME matches expected subdomain", () => {
  ...
  assert.equal(cname, "cert.kineticgain.com");
});
